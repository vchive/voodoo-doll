...
> 最新更新：2026-09-21。MW-36 / SP-21 / MW-AC-36 本机桌面范围完成：修复离线重连丢失/错标、确认丢响应后的取消误判、多标签旧回执覆盖、损坏 pending 锁死和旧离线档导入位置分裂。完整浏览器回归 25/25，最后坐标补丁后相关三项存档场景分别复测通过；Node 112/112、Python 166/166、类型检查与双构建通过。仓库入口 `npm run test:play` 使用临时数据库/随机端口/独立浏览器，不使用用户存档。详见[完整试玩与恢复证据](evidence/2026-09-21-playthrough-recovery.md)。
