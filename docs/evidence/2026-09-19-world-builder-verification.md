...
| 任务 | 需求 / 验收 | 本轮结果 | 尚未覆盖 |
| --- | --- | --- | --- |
| MW-22/MW-25/MW-26 | WL-08/WL-09；MW-AC-22/25/26 | 实体/公开重复日程/遭遇声明校验与预览，哈希/版本复核、原子发布、取消、重启回执、跨连接并发确认；`tool-compiler-3` 已预演自行车单步和地铁三步组合动作 | 新房间/角色、例外日程、私密日程、非零容差、图可达性、分支组合动作、高能力模型工作流和事件从零重建 |
...
工作区 `/Users/liminghan/Documents/voodoo-doll`，macOS 26.6.2 arm64；Python 3.9.6、SQLite 3.51.0、FastAPI 0.128.8、httpx 0.28.1。以下 SHA-256 是 2026-09-20 单人预发布收口时的文件级证据；随后建立的本地 Git 基线为 `418c79b`，当前证据提交为 `57b5fdd`，本地标签为 `prerelease-local-2026-09-20`。提交和标签未在本文证明已推送或部署。HTTP 合同测试使用 FastAPI TestClient，不代表真实公网访问；依赖声明已增加 `backend[test]`。
...
- `tool-compiler-3` 对自行车单步、地铁 `board -> travel -> alight` 做无副作用预演；未上车、未下车、范围外实体、未知实体/工具和错误 affordance 在草稿持久化前拒绝。
...
本轮没有改 UI，没有重新做桌面手机尺寸或微信真机验证；未请求真实模型、未连接部署服务器、未清理已有玩家存档。现有回放主要读取持久快照与事件；从事件独立重建全部状态、mobility 端到端回放仍需单独验收。基础 cookie 会话隔离、写请求 Origin 校验和 internal 管理令牌已有自动化；完整 viewer/session/private 越权矩阵、生产配置和公网发布仍未完成。
