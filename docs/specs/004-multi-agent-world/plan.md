...
本轮状态为实现中/待测；证据填入 [dialogue-focus](../../evidence/2026-09-20-dialogue-focus.md)。本任务不扩写长篇，不新增模型调用，不宣称微信真机或公网通过。
...
Phase 1 默认教程由 MW-28/MW-29（SP-11–14、MW-AC-28/29）、MW-30/MW-31（SP-15/16、MW-AC-30/31）和 MW-33（SP-18、MW-AC-33 的阅读与行动收敛）共同验收；MW-32（SP-17、MW-AC-32）单列 v1 旧档支线兼容。signal 代码、自动化和独立浏览器证据齐全后，才能把单人教程标杆的本机范围标为完成，微信真机、公网和完整故障矩阵继续分别验收。Phase 2/3 的需求不得回写成 Phase 1 已完成能力。每次扩写先更新对应 spec/plan/tasks/acceptance，再实现最小纵切片；任何“已完成”状态必须附真实测试命令、事件或快照样例及设备范围。
