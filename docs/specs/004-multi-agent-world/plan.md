...
前次核查（2026-09-21）：仅核查多 Agent 世界实现状态，见[源码核查与验证范围](../../evidence/2026-09-21-agent-world-status.md)。文末补充后续核心阶段建议，不新增已完成能力。
...
MW-43（SP-28 → MW-AC-43 A–E）接续 MW-10/14/28/31，本机已测范围完成。本轮实现此前核心阶段建议的首个角色切片，不将后续自主生活/监督路由并入完成范围。分流粒度与上下文取舍为实现默认，详见[决策 0011](../../decisions/0011-character-conversation-boundary.md)。

1. **剧情与生活分流**：`signal_story.py` 保留关键剧情/证据/选择的确定性回应与推进，林川普通生活话题才进入现有 Adapter。预览仅解析，不调用角色；确认再建上下文和请求。普通聊天不可因为身处 meeting/hearing 就自动听取证据或选择路线；hearing 只认回执/更正预约话题，不能由“提前/预约”等日常词推进，结局只接受首个完整命令子句中的有限明确命令或推荐，疑问/否定/转述和延后讨论不等于选择承诺。自然时钟与现有行动耗时照常计算，B/C 仍按稿件。同场 A 的 `chat-a-work` / `chat-a-memory` 进入既有自由行动面板，各约 3 分钟，离场不展示；无前端改动。
2. **角色上下文与本地回应**：新增 `backend/app/domain/character_context.py`，由 `perception.py` 只加入本人 persona/traits/goals、当前地点/日程活动与时钟和已隔离记忆。`resolver.py` / `agents/gateway.py` 接通按话题本地 fallback 与真实 Adapter 路径；不把其他角色私密上下文或未解锁秘密放进回复，不让模型改写权威状态。未配置/失败也能聊，本地工作/安排/家人关心/问候/回忆回应已实现；重访仅引用本人可见的已提交玩家原话，不将其声明当作事实。
3. **文件责任与验证**：集成负责人负责 `backend/app/signal_story.py` 与浏览器测试脚本，角色上下文负责人维护上述 context/perception/resolver/gateway；测试负责人新增 `backend/tests/test_character_conversation.py` 并调整受影响旧测试，SDD owner 维护四件套，HANDOFF 和[证据](../../evidence/2026-09-21-character-conversation.md)由集成负责人维护。先证明确认实际调用 Adapter、预览/取消零调用，再验隐私/重访、剧情门槛、故障/幂等和三路线；横屏实际交谈、反馈与回看单独取证。真实供应商未测时不得将注入 transport 当作真实模型验收。

最终后端 183/183、Python 编译、脚本语法与差异检查通过；包含 15 项角色对话测试和 signal 16 项旧测试，独立剧情分流子流程补充核对日常提问、否定与转述。阶段浏览器广泛 49/49 通过；最后收紧结局命令子句边界后，最终相关补测 13/13（3 生活、6 完整路线、4 错过）通过，不能合称最终同版全49。三档生活交谈实际读到工作回应、蓝伞原话回忆及回看；计数 Adapter / mock Gateway transport 和无模型浏览器分别取证，不等于真实供应商质量验证，模型自然语言事实一致性仍待真实 provider。最终范围见[角色对话证据](../../evidence/2026-09-21-character-conversation.md)及[最终13项报告](../../evidence/2026-09-21-character-conversation-browser.json)。

本机服务已备份原SQLite后重启，10表3,557行内容指纹保持；首页、健康与27个静态资源HTTP 200且匹配现有构建，见[本机只读证据](../../evidence/2026-09-21-character-conversation-local.json)。未操作用户故事，无前端产品改动/重建，不计作公网部署。三档生活截图已经人工核对，无裁切阻断；微信真机和真实模型仍另验。
...
以下保留前次状态核查时的实现建议，未由用户逐项确认；MW-43 已完成第一阶段中林川普通生活对话的本机有限验收，其余建议未实施。核查时已有权威世界、角色状态、可见性隔离、时间表与兴趣租约；当时教程同场 A/B/C 对话先被固定稿接管，模型上下文缺少 traits/goals，Kernel 共用统一环境配置的 Gateway。该历史基线见[Agent 世界核查](../../evidence/2026-09-21-agent-world-status.md)，当前 A 接入及未测边界以 MW-43 为准。
