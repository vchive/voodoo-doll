...
当前默认故事为 `rainy-office-v2` 三日教程，`rainy-office-v1` 五章短篇仅保留旧存档兼容和历史回归。前一切片（2026-09-20）的 Python 123/123、Node 83/83、三结局浏览器通关及 320/390 布局仅证明 v1，见[历史证据](../../evidence/2026-09-20-playable-tutorial.md)；v2 不沿用旧短篇的通过结论。

对话窗任务（2026-09-20）：MW-30 / SP-15 已实现用户要求的 Galgame 对话窗，MW-AC-30 本机通过；当前 Python 138/138、Node 93/93、类型检查、Hex 构建和浏览器阅读交互/两条结局路线已有[对话窗证据](../../evidence/2026-09-20-galgame-dialogue.md)。MW-28/29 的短篇代码、无模型路径和自动化已完成，整项验收仍受浏览器/真机/故障矩阵范围限制；阅读演出不代表长篇内容完成。

当前修复任务（2026-09-20）：MW-31 / SP-16 针对真实试玩暴露的非标准顺序卡住、周野重复空泛回应和上下文接话失败，依赖 MW-28–30，状态为代码与自动化回归通过、浏览器待验。纠偏必须由服务端已确认事实驱动，不能把阅读游标或玩家界面记录当作世界事实；未知行动仍拒绝，错误后必须直接显示可执行恢复入口。

教程兼容任务（2026-09-20）：MW-32 / SP-17 仅在 `rainy-office-v1` 旧主线结局后增加第二天可选支线，使用旧伞、失物登记和档案卡片练习多日世界与 NPC 离场后的替代路径。服务端自动化回归通过，浏览器与真机证据待补；支线不改写旧主线 `ending`，也不引入动态工具或运行时代码。v2 第二日地铁站/厨房分支属于 MW-28，不复用 `postscript` 支线进度。

当前状态（2026-09-20）：Python 138/138、Node 93/93、TypeScript 类型检查及 Hex 构建通过。世界草稿的实体/公开日程/遭遇同批确认和并发/取消/重启已有验收；匿名单人闭环、HTTP 写入保护、管理接口令牌与同源静态入口已有自动化，MW-22/23/25/26/27 仍为部分完成。MW-27 的 SP-07–10 已完成代码与 Chrome 390×844 增量验收，包括损坏缓存恢复、快捷行动、11 地点精确移动、防重复、中文错误、离线存档往返、本机进度保护、故事预览恢复、坏导入恢复、确认响应丢失恢复和操作锁。MW-28/29 的原创短篇、推荐动作、主角工作生活及时间分支已完成代码与无模型回归；MW-30、MW-31、MW-32 的代码和后端回归也已完成，浏览器/微信真机/完整网络故障矩阵仍待补。长篇内容、新房间/角色、例外日程、观察者完整不变量/修复仍待完成。每项任务须同时有代码、测试和验收证据，见[世界搭建记录](../../evidence/2026-09-19-world-builder-verification.md)和[单人预发布记录](../../evidence/2026-09-19-single-player-prerelease.md)；不能以迁移表述覆盖 002/003 的未完成项。
...
| MW-28 | 实现可通关的原创 Galgame 新手故事与自定义故事模板 | SP-11, SP-14 | MW-AC-28 | MW-13, MW-17, MW-18, MW-19, MW-27 | 部分完成 / Codex：默认 `rainy-office-v2`《雨停以前》三日教程已有代码及无模型路径基础；第一日通行证/便笺、通勤、校对与林川谈话，第二日地铁站/厨房双入口与档案调查，第三日回执、听证及最终决定，支持 `trust/audit/protect/missed`。服务端保存步骤、线索与选择，保留自定义模板；v2 自动化、浏览器、微信真机、公网及完整故障矩阵须各有新证据。v1 五章与三结局已有历史证据，仅作旧档兼容回归，不能证明 v2 已完整验收 |
| MW-29 | 实现场景推荐动作、未知输入恢复与主人公生活职责 | SP-12, SP-13 | MW-AC-29 | MW-17, MW-18, MW-19, MW-23, MW-27 | 部分完成 / Codex：默认 v2 复用服务端 guidance 的场景/目标推荐、自由表达、开门等已发布实体交互与明确未知输入拒绝，主人公通勤与校对的耗时和完成状态持久保存。v1 的预览/取消/重试、时间边界自动化和原话开门、未知输入恢复、工作完成、窄屏浏览器为已有基线；v2 三日内的推荐动作与生活状态仍需独立自动化及浏览器证据，微信真机、公网和完整网络故障矩阵待补 |
| MW-30 | 实现 Galgame 对话窗、多句演出与阅读恢复 | SP-15 | MW-AC-30 | MW-28, MW-29 | 已完成（本机范围）/ Codex：公开 guidance 对话标识与对白/主角心声/环境数组、角色名牌、逐句阅读、本段回看、读完选项及提前行动已接通。阅读按世界/段落恢复，NPC 私密思考不投影，预览期间禁用阅读且取消保留位置。Python 138/138、Node 93/93、类型检查与 Hex 构建通过；相信/追问两路线及 320/390 阅读交互已实玩，见[对话窗证据](../../evidence/2026-09-20-galgame-dialogue.md)。微信真机与完整故障矩阵未扩展 |
| MW-31 | 修复教程事实推进、上下文接话与错误恢复 | SP-16 | MW-AC-31 | MW-28, MW-29, MW-30 | 代码与自动化回归通过、浏览器待验：根据服务端已确认事实接受先开门后读便笺、合法离场等顺序；旧错位存档以可审计幂等事务纠偏，不补线索、不重开机会、不重复对白。周野对日常、站内告示、三分钟和后续追问给出具体回应；“说什么”“什么意思”“然后呢”“继续说”等接话优先关联最近同场对象，唯一对象可补全，多对象需澄清，离场对象失效。未知行动仍拒绝；解析错误保留原文和记录，直接展开推荐动作、可交谈人物及恢复提示。失败输入固定追加`你 · 未执行`与`巫柜 · 提示`两条本地记录，刷新可恢复且不推进时钟、地点、章节或世界事件；新行动记录显示发生时的日期、时间、地点与执行状态，并以 turn/event id 防止确认回放重复追加，旧日志保持兼容。 |
| MW-32 | 保留 v1 第二天旧伞支线与旧档恢复 | SP-17 | MW-AC-32 | MW-28, MW-29, MW-30, MW-31 | 仅 v1 兼容范围：代码与后端回归通过、浏览器待验。旧主线三种结局后可从推荐动作进入 `postscript`；回地铁站询问周野写入 `station-pass`，周野离场后观察登记写入 `station-pass-late`，办公室检查写入 `archive-card`，回家开门完成支线。支线保持原 `ending`，预览/取消无副作用，导出/导入保留步骤；缺少 `postscript` 的 v1 旧档仍可导入并继续。无模型路径由固定规则完成；320/390 桌面、微信真机、公网和故障矩阵待验。v2 第二日主线另属 MW-28。 |
...
- `MW-28` 属于 `backend/app/{gameplay,narrative}.py`、Kernel 的动作事务、`hex/{play,play-state,play-api}.ts`、`hex/play.css` 及对应测试；章节与分支在现有世界 metadata/存档中持久化，不能由本机步骤代替服务端结果。故事内容见 `tutorial-story.md`。
...
- `MW-31` 属于 `backend/app/{gameplay,narrative}.py`、教程/对话上下文解析与事务测试、`hex/{play,play-api}.ts` 及错误恢复展示；服务端以已确认事件、WorldClock、PresenceProjection 和当前公开对话建立进度与接话目标，前端只展示 guidance、保留输入和恢复展开状态。纠偏事件必须可审计、幂等且不改变既有时间窗/结局；SDD owner 只改本目录五份规格/故事文档，集成负责人维护 HANDOFF 与证据。
- `MW-32` 属于 `backend/app/{gameplay,narrative}.py`、`backend/tests/test_narrative.py`、`hex/{play,play-api}.ts` 及教程支线证据；它只维护 v1 兼容存档，复用 MW-28–31 的章节、行动、对话和错误恢复边界，不改变旧主线 `ending`，不新增运行时动态工具。v2 第二日主线由 MW-28 维护。SDD owner 只改本目录五份规格/故事文档，集成负责人维护 HANDOFF 与证据。

## 依赖阶段

`MW-01 -> MW-02/MW-03 -> MW-04 -> MW-05/MW-06 -> MW-07 -> MW-08 -> MW-09 -> MW-10/MW-11 -> MW-12/MW-13 -> MW-14/MW-15 -> MW-17 -> MW-18 -> MW-19 -> MW-20/MW-21 -> MW-22 -> MW-23 -> MW-24/MW-25 -> MW-26 -> MW-27 -> MW-28/MW-29 -> MW-30 -> MW-31；MW-32（v1 旧档兼容，依赖 MW-28–31） -> MW-16`。
